...
from .modeling_mamba import MambaBlock, MambaRMSNorm, MambaModel
from .configuration_mamba import MambaConfig

class TransformerInterfaceMambaEncoderTransDecoder(nn.Module):
    def __init__(
        self,
        d_model=512,
        nhead=8,
        ...
        encoder_module: Optional[str] = "mamba",
        ...
        mamba_d_conv: Optional[int] = 256,
        mamba_d_expand: Optional[int] = 256,
    ):
        super().__init__()
        self.causal = causal
        ...
        if num_encoder_layers > 0:
            ...
            if encoder_module == "mamba":
                self.encoder = TransformerMambaAttentionEncoder(
                    nhead=nhead,
            ...
            elif encoder_module == "mambaAttmamba":
                self.encoder = TransformerMambaAttentionMambaEncoder(
                    nhead=nhead,
                    num_layers=num_encoder_layers,
                    d_ffn=d_ffn,
                    d_model=d_model,
            ...
            elif encoder_module == "mambaOnly":
                self.encoder = TransformerMambaEncoder(
                    nhead=nhead,
                    num_layers=num_encoder_layers,
                    d_ffn=d_ffn,
                    d_model=d_model,
...
class TransformerInterfaceMambaAttentionEncoderDecoder(nn.Module):
    def __init__(
        self,
        d_model=512,
        nhead=8,
        ...
        if num_decoder_layers > 0:
            if custom_tgt_module is not None:
                self.custom_tgt_module = custom_tgt_module(d_model)
            self.decoder = TransformerMambaOnlyDecoderAll(
                num_layers=num_decoder_layers,
                nhead=nhead,
                d_ffn=d_ffn,
                d_model=d_model,
    ...
    def forward(self, **kwags):
        """Users should modify this function according to their own tasks."""
        raise NotImplementedError
class TransformerMambaAttentionEncoder(nn.Module):
    def __init__(
        self,
        num_layers,
        nhead,
        ...
        self.layers = torch.nn.ModuleList(
            [
                TransformerMambaAttEncoderLayer(
                    d_ffn=d_ffn,
                    nhead=nhead,
                    d_model=d_model,
                    kdim=kdim,
...
class TransformerMambaEncoder(nn.Module):
    def __init__(
        self,
        num_layers,
        nhead,
...
class TransformerMambaAttentionMambaEncoder(nn.Module):
    def __init__(
        self,
        num_layers,
        nhead,
        ...
        self.layers = torch.nn.ModuleList(
            [
                TransformerMambaAttMambaEncoderLayer(
                    d_ffn=d_ffn,
                    nhead=nhead,
                    d_model=d_model,
                    kdim=kdim,
    ...
    def forward(
        ...
        output = self.norm(output)
        return output
class TransformerMambaAttEncoderLayer(nn.Module):
    def __init__(
        self,
        d_ffn,
        nhead,
        ...
        configuration.state_size = mamba_d_state
        self.mamba_layers1 = nn.ModuleList([MambaBlock(config=configuration, layer_idx=idx) for idx in range(mamba_d_expand)])
    ...
    def forward(
        ...
        # add & norm
        output = src + self.dropout1(output)
        for mixer_block in self.mamba_layers1:
            output = mixer_block(output)
        return output

class TransformerMambaEncoderLayer(nn.Module):
    def __init__(
        self,
        d_ffn,
        nhead,
...
class TransformerMambaAttMambaEncoderLayer(nn.Module):
    def __init__(
        self,
        d_ffn,
        nhead,
        ...
        configuration = MambaConfig()
        configuration.state_size = mamba_d_state
        self.mamba_norm = MambaRMSNorm(configuration.hidden_size, eps=configuration.layer_norm_epsilon)
        self.mamba_layers1 = nn.ModuleList([MambaBlock(config=configuration, layer_idx=idx) for idx in range(mamba_d_conv)])
        self.mamba_layers2 = nn.ModuleList(
            [MambaBlock(config=configuration, layer_idx=idx) for idx in range(mamba_d_expand)])
    def forward(
        self,
        src,
        src_mask: Optional[torch.Tensor] = None,
        ...
        # add & norm
        output = src + self.dropout1(output)
        for mixer_block in self.mamba_layers2:
            output = mixer_block(output)
        return output

class TransformerMambaOnlyDecoderLayerAll(nn.Module):
    def __init__(
        self,
        d_ffn,
        nhead,
        ...
        configuration.state_size = mamba_d_state
        self.mamba_layers1 = nn.ModuleList([MambaBlock(config=configuration, layer_idx=idx) for idx in range(mamba_d_conv)])
        self.mamba_layers2 = nn.ModuleList([MambaBlock(config=configuration, layer_idx=idx) for idx in range(mamba_d_expand)])
        self.mamba_norm = MambaRMSNorm(configuration.hidden_size, eps=configuration.layer_norm_epsilon)
    ...
    def forward(
        ...
        # add & norm
        tgt1 = tgt + self.dropout(tgt2)
        for mixer_block in self.mamba_layers2:
            tgt = mixer_block(tgt1)

        return tgt, multihead_attention

class TransformerMambaOnlyDecoderAll(nn.Module):
    def __init__(
        self,
        num_layers,
        nhead,
        ...
        self.layers = torch.nn.ModuleList(
            [
                TransformerMambaOnlyDecoderLayerAll(
                    d_ffn=d_ffn,
                    nhead=nhead,
                    d_model=d_model,
                    kdim=kdim,
    ...
    def forward(
        ...
        for dec_layer in self.layers:
            output, multihead_attn = dec_layer(
                output,
                memory,
                tgt_mask=tgt_mask,
